...
with col2:
    if st.button("➖ Quitar última fuerza"):
        if len(st.session_state.forces) > 2:
...
if st.button("Calcular distribución rectangular"):
    ...
    for i in range(len(forces)):
        st.write(f"**{names[i]} = {forces[i]:.4f} N**    st.subheader("Verificación")

    st.info(f"∑F = {sumF:.10f} N")
    st.info(f"∑M = {sumM:.10f} Nm")
    st.info(f"Error momento = {abs(sumM - moment_expected):.10e} Nm")

    # Gráfico
    fig, ax = plt.subplots(figsize=(12, 5))

    max_force = max(abs(forces)) if np.max(abs(forces)) > 0 else 1
